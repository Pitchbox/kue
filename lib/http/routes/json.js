...
function getSearch() {
    ...
    var client = reds.createClient();
    return search = reds.createSearch(client.getKey('search'));
};
...
exports.createJob = function (req, res) {
    ...
    // If array of jobs is passed, batch insert them all
    if(body instanceof Array) {
        var len = body.length;
        var jobs = [];

        if (len === 0) {
            return res.send({ error: 'Empty array' }, 500);
        }

        // Build array of jobs while checking for obvious errors
        for (var i = 0; i < len; i++) {
            jobs.push(parseJob(body[i]));
        };
        ...
        for (var i = 0; i < len; i++) {
            (function(i) {
                ...
                job.save(function(err) {
                    savedCount += 1;
                    if (err) {
                        return res.send({ error: err.message, ids: ids }, 500);
                    } else {
                        ids[i] = job.id;
                    }
                    if (savedCount === len) {
